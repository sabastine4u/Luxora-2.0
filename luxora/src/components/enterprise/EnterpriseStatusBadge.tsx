...
export function EnterpriseStatusBadge({ status }: EnterpriseStatusBadgeProps) {
  const getStatusColor = (s: string) => {
    ...
    if (['active', 'completed', 'approved', 'paid', 'success', 'open', 'published', 'verified', 'assigned to agency', 'agency acknowledged', 'online', 'operational', 'optimal', 'healthy', 'sold', 'rented'].includes(normalized)) {
    ...
    if (['draft', 'inactive', 'on hold', 'cancelled', 'hidden', 'archived'].includes(normalized)) {
      return 'text-ink/60 bg-white/5 border-white/10';
    }
    if (['featured'].includes(normalized)) {

import { Leaf, Star, Search, ShieldCheck } from 'lucide-react';
import { GoldButton } from '../../../components/ui/ui';

export default function Landscaping() {
  const providers = [
    { id: 'LND-1', name: 'Eden Gardens', rating: '4.8', reviews: 215, price: 'Custom Quote', tags: ['Design', 'Maintenance', 'Hardscaping'] },
    { id: 'LND-2', name: 'GreenThumb Pros', rating: '4.6', reviews: 142, price: 'From ₦50,000/month', tags: ['Lawn Care', 'Pool Maintenance'] },
  ];
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-heading text-2xl font-bold text-cream">Landscaping & Outdoors</h2>
          <p className="text-sm text-ink/60">Professional landscapers and pool maintenance specialists.</p>
        </div>
      </div>
        ...
        {providers.map((p) => (
          <div key={p.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-2xl border border-white/10 bg-navy-800/50 hover:border-gold-400/30 transition-colors">
            <div className="flex items-center gap-4 mb-4 sm:mb-0">
               <div className="h-12 w-12 rounded-xl bg-emerald-400/10 flex items-center justify-center text-emerald-400 shrink-0">

import { Shield, Star, Search, ShieldCheck } from 'lucide-react';
import { GoldButton } from '../../../components/ui/ui';

export default function Security() {
  const providers = [
    { id: 'SEC-1', name: 'IronClad Security', rating: '5.0', reviews: 412, price: 'Custom Quote', tags: ['Guards', 'CCTV', 'Access Control'] },
    { id: 'SEC-2', name: 'SafeHome Tech', rating: '4.8', reviews: 156, price: 'From ₦150,000', tags: ['Alarms', 'Smart Locks'] },
  ];
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-heading text-2xl font-bold text-cream">Security & Surveillance</h2>
          <p className="text-sm text-ink/60">Protect your property with top-tier security personnel and systems.</p>
        </div>
      </div>
        ...
        {providers.map((p) => (
          <div key={p.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-2xl border border-white/10 bg-navy-800/50 hover:border-gold-400/30 transition-colors">
            <div className="flex items-center gap-4 mb-4 sm:mb-0">
               <div className="h-12 w-12 rounded-xl bg-rose-400/10 flex items-center justify-center text-rose-400 shrink-0">

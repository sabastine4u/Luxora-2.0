import { Zap, Star, Search, ShieldCheck } from 'lucide-react';
import { GoldButton } from '../../../components/ui/ui';

export default function Electrical() {
  const providers = [
    { id: 'ELC-1', name: 'Volt Masters', rating: '4.8', reviews: 205, price: 'From ₦15,000/callout', tags: ['Repairs', 'Installations', '24/7'] },
    { id: 'ELC-2', name: 'SafeWire Engineers', rating: '4.9', reviews: 110, price: 'From ₦20,000/callout', tags: ['Certifications', 'Rewiring'] },
  ];
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-heading text-2xl font-bold text-cream">Electrical Services</h2>
          <p className="text-sm text-ink/60">Certified electricians for installations and emergency repairs.</p>
        </div>
      </div>
        ...
        {providers.map((p) => (
          <div key={p.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-2xl border border-white/10 bg-navy-800/50 hover:border-gold-400/30 transition-colors">
            <div className="flex items-center gap-4 mb-4 sm:mb-0">
               <div className="h-12 w-12 rounded-xl bg-yellow-400/10 flex items-center justify-center text-yellow-400 shrink-0">
            ...
            <GoldButton>Request Service</GoldButton>
          </div>
        ))}
      </div>
    </div>

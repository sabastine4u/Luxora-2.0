import { Building2, Star, Search, ShieldCheck } from 'lucide-react';
import { GoldButton } from '../../../components/ui/ui';

export default function Furniture() {
  const providers = [
    { id: 'FURN-1', name: 'Artisan Woodworks', rating: '4.8', reviews: 112, price: 'Custom Quote', tags: ['Bespoke', 'Local Materials', 'Fitted'] },
    { id: 'FURN-2', name: 'Modern Living Furniture', rating: '4.7', reviews: 340, price: 'Catalog Prices', tags: ['Ready-made', 'Assembly'] },
  ];
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-heading text-2xl font-bold text-cream">Furniture & Fitting</h2>
          <p className="text-sm text-ink/60">Bespoke joinery, ready-made furniture, and assembly services.</p>
        </div>
      </div>
        ...
        {providers.map((p) => (
          <div key={p.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-2xl border border-white/10 bg-navy-800/50 hover:border-gold-400/30 transition-colors">
            <div className="flex items-center gap-4 mb-4 sm:mb-0">
               <div className="h-12 w-12 rounded-xl bg-amber-400/10 flex items-center justify-center text-amber-400 shrink-0">
            ...
            <GoldButton>View Catalog</GoldButton>
          </div>
        ))}
      </div>
    </div>

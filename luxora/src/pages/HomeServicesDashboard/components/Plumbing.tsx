import { Droplet, Star, Search, ShieldCheck } from 'lucide-react';
import { GoldButton } from '../../../components/ui/ui';

export default function Plumbing() {
  const providers = [
    { id: 'PLM-1', name: 'AquaFix Plumbers', rating: '4.7', reviews: 188, price: 'From ₦12,000/callout', tags: ['Emergencies', 'Piping'] },
    { id: 'PLM-2', name: 'Lagos Water Works', rating: '4.6', reviews: 75, price: 'From ₦10,000/callout', tags: ['Heaters', 'Drainage'] },
  ];
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-heading text-2xl font-bold text-cream">Plumbing Services</h2>
          <p className="text-sm text-ink/60">Expert plumbers for repairs, installations, and emergencies.</p>
        </div>
      </div>
        ...
        {providers.map((p) => (
          <div key={p.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 rounded-2xl border border-white/10 bg-navy-800/50 hover:border-gold-400/30 transition-colors">
            <div className="flex items-center gap-4 mb-4 sm:mb-0">
               <div className="h-12 w-12 rounded-xl bg-cyan-400/10 flex items-center justify-center text-cyan-400 shrink-0">

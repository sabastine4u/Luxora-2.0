import { User, Mail, Bell, Shield, KeyRound, Smartphone, Briefcase } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-ink/60">Manage your profile, security, and agency details.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-white/10 bg-navy-800/50 p-6 lg:col-span-2 space-y-6">
          ...
          <div className="pt-4 border-t border-white/10 space-y-4">
            <h3 className="font-heading text-lg font-semibold text-cream">Agency Details</h3>
            <div className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold text-cream">Meridian Luxury</div>
                  <div className="text-xs text-ink/50">Senior Agent</div>
                </div>
              </div>
              <span className="text-xs font-medium text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-md">Verified</span>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-white/10 bg-navy-800/50 p-6 space-y-4">
            ...
            {[
              { label: 'New Lead Alerts', desc: 'Notify immediately via Push', icon: Bell, on: true },
              { label: 'Message Alerts', desc: 'Email digest for offline messages', icon: Mail, on: true },
              { label: 'Appointment Reminders', desc: 'SMS 1hr before meeting', icon: Smartphone, on: false },
            ].map((pref, i) => (
